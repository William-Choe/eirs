...
def conv(inputs, kernel_size, num_outputs, name,
        stride_size = 1, padding = 'SAME', activation_fn = tf.nn.relu):
    """
...
def conv2d(inputs, kernel_size, num_outputs, name,
...
def conv_btn(inputs, kernel_size, num_outputs, name,
        is_training = True, stride_size=2 , padding = 'SAME', activation_fn = tf.nn.relu):
    """
    ...
    with tf.variable_scope(name):
        ...
        return outputs
def conv_btn2d(inputs, kernel_size, num_outputs, name,
...
def conv_btn1(inputs, kernel_size, num_outputs, name,
        is_training = True, stride_size=1 , padding = 'SAME', activation_fn = tf.nn.relu):
    """
    Convolution layer followed by batch normalization then activation fn:
    ----------
    Args:
...
def deconv(inputs, kernel_size, num_filters_in, num_outputs, name,
    ...
    with tf.variable_scope(name):
        kernel_shape = [kernel_size, num_outputs, num_filters_in]
        stride_shape = stride_size
        input_shape  = tf.shape(inputs)
        print (input_shape)
        output_shape = tf.stack([input_shape[0], input_shape[1], num_outputs])

        weights = tf.get_variable('weights', kernel_shape, tf.float32, xavier_initializer())
        bias    = tf.get_variable('bias', [num_outputs], tf.float32, tf.constant_initializer(0.0))
        conv_trans = tf.contrib.nn.conv1d_transpose(inputs, weights, output_shape, stride_shape, padding = padding)
...
def deconv_upsample2D(inputs, factor, name, padding = 'SAME', activation_fn = None):
    """
    ...
    with tf.variable_scope(name):
        ...
        weights = bilinear_upsample_weights2d(factor, num_filters_in)
...
def bilinear_upsample_weights2d(factor, num_outputs):
    """
...
def deconv_upsample(inputs, factor, name, padding = 'SAME', activation_fn = None):
    ...
    with tf.variable_scope(name):
        ...
        print ("input_shape,output_shape",input_shape,output_shape)
        weights = bilinear_upsample_weights(factor, num_filters_in)
        print ("num_filters_in, output_shape,weights",num_filters_in, output_shape,weights)
        outputs = tf.contrib.nn.conv1d_transpose(inputs, weights, output_shape, stride_shape, padding = padding)
...
def flatten(inputs, name):
    ...
    with tf.variable_scope(name):
        dim     = inputs.get_shape()[1:3].num_elements()
...
def maxpool(inputs, kernel_size, name, padding = 'SAME'):
    ...
    outputs = tf.layers.max_pooling1d(inputs, pool_size = kernel_shape,
            strides = kernel_shape, padding = padding, name = name)
    return outputs


def maxpool2d(inputs, kernel_size, name, padding='SAME'):
    """
    Max pooling layer:
    ----------
    Args:
...
def concat(inputs1, inputs2, name):
    ...
    print (tf.concat(values=[inputs1, inputs2],axis =2, name = name))

def concat2d(inputs1, inputs2, name):
    """
    Concatente two tensors:
    ----------
    Args:
